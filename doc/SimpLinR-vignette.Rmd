...
Simple linear regression is the process of take a set of inputs $\boldsymbol{x}$ and relating them to a set out outputs $\boldsymbol{y}$ via a line. Consider the the following data.
...
It is not easy to see the relationship for the table above, other than y tends to increases as x increases. Let's plot the data for better visualization.

```{r}
plot(x, y)
```

Using `SimpLinR` we can fit a line to this data that minimizes the sum of quared residuals, plotted in red below.
...
The confidence intervals indicate that the slop appears positive based on the data while, at a 95% confidence level, we would fail to reject the null hypothesis that the y-intercept is greater than zero.

`SimpLinR()` runs on RccpArmadillo. This makes the the function faster than standard linear regression tools such as `lm()`. The C++ code is included below for reference.
